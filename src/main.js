...
function initCinematicSkyHero() {
  ...
  function loop() {
    ...
    // Moon Orbital Revolution & Spiral Zoom equations (Slowing down rotation & spiral)
    const spin = Math.min(1, p / 0.90);
    const theta = spin * Math.PI * 3.5 - Math.PI / 2;

    const merge = ramp(p, 0.50, 0.95);
    const radius = 40 * (1 - merge); // vmin -> 0 at end (Moon center overlaps Earth center!)
    ...
    const zoom = 0.12 + ramp(p, 0.5, 1) * 0.44;

    // Earth stays visible as Moon merges on top, then gently fades out
    const earthOut = ramp(p, 0.65, 0.98);
    ...
    if (moonImg) {
      ...
      moonImg.style.transform = `translate3d(${x}vmin, ${y}vmin, 0) scale(${zoom * orbitScale})`;
    ...
    if (startTitle) {
      const startOut = ramp(p, 0.15, 0.50);
      startTitle.style.opacity = (1 - startOut).toFixed(4);
      startTitle.style.transform = `translateY(${-startOut * 35}px)`;
    }
    if (endTitle) {
      const endIn = ramp(p, 0.55, 0.92);
      endTitle.style.opacity = endIn.toFixed(4);
      endTitle.style.transform = `translateY(${(1 - endIn) * 35}px)`;
    }
